...
<script type="text/javascript">
  ...
  import { getAllUsers,addPurchaseOrder,getOneGoods,getZsmInfo,getGoods,getUser,getAllOrgs,getAllSupplier } from '../../../api/api'
  export default {
    mounted(){
        ...
        getAllStocks(){
          getAllStock({
            ...
            res.data.forEach((item,index) => {
              ...
              temp.value = item.id
              temp.label = item.name
              this.purchaseOrderObj.allstockslist.push(temp)
            })
          })
        },
        /*根据扫描得到的码获取商品信息*/
